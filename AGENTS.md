...
- **First-run data**: after `DATABASE_URL` is set, run `npm run db:push` (creates schema), then `npm run db:seed -- user_<yourClerkId>` to load demo banking/wealth data for a specific signed-in Clerk user. The seed key off the Clerk user ID; without seeding a freshly provisioned user only gets empty default checking/savings accounts (auto-created on first authenticated visit in `src/lib/auth.ts`).
